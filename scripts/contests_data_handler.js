...
exports.manage = function(){
  fc_update();
  var now = moment();
  var passed_contests = new Array();

  for(var i = 0;i < fc.length;i++){
    if(Number(fc[i]["date"].replace(/(\/)|(\s+)|:/g,"")) <= Number(now.format("YYYYMMDDhhmm"))){
      passed_contests.push(fc[i]["name"]);
    }
  }

  if(passed_contests.length){
    this.del(passed_contests);
  }
}
